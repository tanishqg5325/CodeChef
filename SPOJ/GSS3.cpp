...
void update(int si, node st[], int ss, int se, int idx, int val)
{
    if(ss == se) {st[si] = {val, val, val, val}; return;}
    int mid = (ss + se)/2;
...
int main()
{
    ...
    {
        cin>>k>>x>>y;
        if(k == 1) cout<<solve(0, 0, n-1, st, x-1, y-1).ans<<"\n";
        else update(0, st, 0, n-1, x-1, y);
    }
    return 0;
}
